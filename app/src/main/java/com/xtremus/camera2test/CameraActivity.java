package com.xtremus.camera2test;
...
public class CameraActivity extends AppCompatActivity implements EasyPermissions.PermissionCallbacks {
    ...
    private static Size chooseOptimalSize(Size[] choices, int width, int height, Size aspectRatio) {
        ...
        if (bigEnough.size() > 0) {
            ...
            Log.e(TAG, "chooseOptimalSize(): " + choices[0]);
            return choices[0];
        }
    }
    ...
    private void openCamera(int width, int height) {
        ...
        try {
            ...
            Toast.makeText(context,"Camera2API isn't supported on this device",Toast.LENGTH_SHORT).show();
        } catch (InterruptedException e) {
    ...
    @AfterPermissionGranted(REQUEST_VIDEO_PERMISSIONS)
    private void permissionTask() {
        String[] perms = VIDEO_PERMISSIONS;
        if (!EasyPermissions.hasPermissions(this, perms)) {
            // Do not have permissions, request them now
            EasyPermissions.requestPermissions(this, "Video permissions are required to continue!",
                    REQUEST_VIDEO_PERMISSIONS, perms);
        }
